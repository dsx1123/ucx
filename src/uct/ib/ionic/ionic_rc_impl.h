...
#ifndef UCT_IONIC_RC_IMPL_H
#define UCT_IONIC_RC_IMPL_H
...
static inline void
uct_ionic_rc_txqp_posted(uct_rc_txqp_t *txqp, uct_ionic_rc_txcnt_t *txcnt,
...
void uct_ionic_rc_iface_common_progress_enable(uct_iface_h tl_iface,
                                               unsigned flags);

unsigned uct_ionic_rc_iface_post_recv_always(uct_ionic_rc_iface_t *iface,
                                             unsigned max);

static inline unsigned
uct_ionic_rc_iface_post_recv_common(uct_ionic_rc_iface_t *iface, int fill)
{
    ...
    return uct_ionic_rc_iface_post_recv_always(iface, count);
}

static UCS_F_ALWAYS_INLINE void
uct_ionic_rc_iface_handle_am(uct_rc_iface_t *iface, uct_rc_hdr_t *hdr,
...
uct_ionic_rc_iface_poll_rx_common(uct_ionic_rc_iface_t *iface)
{
    uct_ib_iface_recv_desc_t *desc;
    uct_rc_hdr_t *hdr;
    unsigned i;
    ...
    for (i = 0; i < num_wcs; i++) {
        ...
        uct_ionic_rc_iface_handle_am(&iface->super, hdr, wc[i].wr_id, wc[i].qp_num,
...
out:
    uct_ionic_rc_iface_post_recv_common(iface, 0);
    return num_wcs;
}

static UCS_F_ALWAYS_INLINE void
uct_ionic_rc_iface_fill_inl_sge(uct_ionic_rc_iface_t *iface, const void *addr0,
...
static inline void
uct_ionic_rc_iface_fill_inl_am_sge(uct_ionic_rc_iface_t *iface,
                                   ...
                                   const void *buffer, unsigned length)
{
    ...
    uct_ionic_rc_iface_fill_inl_sge(iface, am, sizeof(*am), buffer, length);
}

static inline void
uct_ionic_rc_iface_fill_inl_am_sge_iov(uct_ionic_rc_iface_t *iface, uint8_t id,
...
#define UCT_IONIC_RC_IFACE_FOREACH_TXWQE(_iface, _i, _wc, _num_wcs) \
...
#define UCT_IONIC_RC_FILL_INL_PUT_WR(_iface, _raddr, _rkey, _buf, _len) \
...
#define UCT_IONIC_RC_FILL_SGE(_wr, _sge, _length) \
    _wr.sg_list = &_sge; \
    _wr.num_sge = 1; \
    _sge.length = _length;

#define UCT_IONIC_RC_FILL_AM_BCOPY_WR(_wr, _sge, _length, _wr_opcode) \
    UCT_IONIC_RC_FILL_SGE(_wr, _sge, _length) \
    _wr_opcode = (typeof(_wr_opcode))IBV_WR_SEND;

#define UCT_IONIC_RC_FILL_AM_ZCOPY_WR_IOV(_wr, _sge, _iovlen, _wr_opcode) \
    _wr.sg_list = _sge; \
    _wr.num_sge = _iovlen; \
    _wr_opcode  = (typeof(_wr_opcode))IBV_WR_SEND;

#define UCT_IONIC_RC_FILL_RDMA_WR(_wr, _wr_opcode, _opcode, \
...
#define UCT_IONIC_RC_FILL_RDMA_WR_IOV(_wr, _wr_opcode, _opcode, _sge, _sgelen, \
                                      _raddr, _rkey) \
    _wr.wr.rdma.remote_addr = _raddr; \
    _wr.wr.rdma.rkey        = _rkey; \
    _wr.sg_list             = _sge; \
...
#define UCT_IONIC_RC_FILL_DESC_WR(_wr, _desc) \
    { \
        struct ibv_sge *sge; \
        (_wr)->next    = NULL; \
        sge            = (_wr)->sg_list; \
